...
# net.add_node(ExponentialNode("Exp1", tau=0.4))
...
#     dt=dt
# )
# net.add_node(FilteredExponentialNode("GaussianNode", tau=0.3, filter_kernel=kernel, dt=dt))
...
# tau = 0.2
# net.add_node(FilteredExponentialNode("FiltExp", tau=tau, filter_kernel=kernel, dt=dt, noise_level=0.01))
...
# freq = 1.0
# initial_state = 0.0
# net.add_node(OscillatorNode("Osc1", tau=tau, freq=freq, initial_state=initial_state))
# net.add_node(FilteredOscillatorNode("FiltOsc", tau=2.0, freq=5.0, filter_kernel=np.sin(np.linspace(0, np.pi, 100)), dt=dt))
